...
const QrCodeGenerator = () => {
  ...
  const renderInputFields = () => {
    const inputClass = "h-11 text-sm border-[hsl(230,20%,88%)] focus:border-[hsl(230,80%,55%)] focus:ring-[hsl(230,80%,55%)]";
    switch (qrType) {
      case "website":
        return (
          <div>
      ...
      case "text":
        return (
          <div>
            ...
            <Textarea placeholder="Enter any text or message..." value={plainText} onChange={(e) => setPlainText(e.target.value)} className="min-h-[100px] text-sm border-[hsl(230,20%,88%)]" maxLength={2000} />
              ...
              <div className="flex gap-2">
                {["WPA", "WEP", "nopass"].map((enc) => (
                  <button key={enc} onClick={() => setWifiEncryption(enc)}
                    className={`px-4 py-2 rounded-lg text-sm font-medium transition-all ${wifiEncryption === enc ? "bg-[hsl(230,80%,55%)] text-white" : "bg-[hsl(230,20%,95%)] text-[hsl(230,15%,40%)] hover:bg-[hsl(230,20%,90%)]"}`}
      ...
      case "email":
        return (
          <div className="space-y-3">
            <div><Label className="text-sm text-[hsl(230,15%,40%)] mb-1.5 block">Email Address</Label><Input placeholder="example@email.com" value={emailTo} onChange={(e) => setEmailTo(e.target.value)} className={inputClass} /></div>
            <div><Label className="text-sm text-[hsl(230,15%,40%)] mb-1.5 block">Subject</Label><Input placeholder="Email subject" value={emailSubject} onChange={(e) => setEmailSubject(e.target.value)} className={inputClass} /></div>
            <div><Label className="text-sm text-[hsl(230,15%,40%)] mb-1.5 block">Body</Label><Textarea placeholder="Email body..." value={emailBody} onChange={(e) => setEmailBody(e.target.value)} className="min-h-[80px] text-sm border-[hsl(230,20%,88%)]" /></div>
          </div>
        );
      case "phone":
        return (<div><Label className="text-sm text-[hsl(230,15%,40%)] mb-1.5 block">Phone Number</Label><Input placeholder="+91 9876543210" value={phoneNumber} onChange={(e) => setPhoneNumber(e.target.value)} className={inputClass} /></div>);
      case "sms":
        return (
          <div className="space-y-3">
            <div><Label className="text-sm text-[hsl(230,15%,40%)] mb-1.5 block">Phone Number</Label><Input placeholder="+91 9876543210" value={smsNumber} onChange={(e) => setSmsNumber(e.target.value)} className={inputClass} /></div>
            <div><Label className="text-sm text-[hsl(230,15%,40%)] mb-1.5 block">Message</Label><Textarea placeholder="SMS message..." value={smsMessage} onChange={(e) => setSmsMessage(e.target.value)} className="min-h-[80px] text-sm border-[hsl(230,20%,88%)]" /></div>
          </div>
        );
    }
  };
    ...
    <div className="min-h-screen bg-[hsl(230,25%,97%)]">
      ...
      <div className="bg-white border-b border-[hsl(230,20%,90%)]">
        <div className="max-w-6xl mx-auto px-4 py-4 text-center">
          <div className="flex items-center justify-center gap-2.5 mb-1">
            <div className="w-8 h-8 rounded-lg bg-[hsl(230,80%,55%)] flex items-center justify-center">
              <QrCode className="w-4.5 h-4.5 text-white" />
            </div>
            <h1 className="text-lg md:text-xl font-bold text-[hsl(230,30%,15%)]">QR Code Generator</h1>
          </div>
          <p className="text-[hsl(230,15%,55%)] text-xs">Customize with your color, shape and logo in 3 simple steps</p>
        </div>
      </div>
      ...
      <div className="max-w-6xl mx-auto px-4 py-5 space-y-5">
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-[hsl(230,20%,92%)] overflow-hidden">
          <div className="flex overflow-x-auto border-b border-[hsl(230,20%,92%)]">
            {QR_TYPES.map((type) => {
              const Icon = type.icon;
              const isActive = qrType === type.id;
              return (
                <button key={type.id} onClick={() => setQrType(type.id)}
                  className={`flex items-center gap-1.5 px-4 py-3 text-sm font-medium whitespace-nowrap transition-all border-b-2 ${isActive ? "border-[hsl(230,80%,55%)] text-[hsl(230,80%,55%)] bg-[hsl(230,80%,97%)]" : "border-transparent text-[hsl(230,15%,50%)] hover:text-[hsl(230,30%,30%)]"}`}
                >
          ...
          <div className="flex flex-col lg:flex-row">
            {/* Left Panel */}
            <div className="flex-1 border-r-0 lg:border-r border-[hsl(230,20%,92%)]">

              {/* Mobile: Content/Design toggle */}
              <div className="lg:hidden flex border-b border-[hsl(230,20%,92%)]">
                <button onClick={() => setMobileStep("content")}
                  className={`flex-1 flex items-center justify-center gap-2 py-3 text-sm font-medium transition-all ${mobileStep === "content" ? "bg-[hsl(230,80%,97%)] text-[hsl(230,80%,55%)] border-b-2 border-[hsl(230,80%,55%)]" : "text-[hsl(230,15%,50%)]"}`}
                >
                  <span className="w-5 h-5 rounded-full bg-[hsl(230,80%,55%)] text-white text-[10px] font-bold flex items-center justify-center">1</span>
                  Content
                </button>
                <button onClick={() => setMobileStep("design")}
                  className={`flex-1 flex items-center justify-center gap-2 py-3 text-sm font-medium transition-all ${mobileStep === "design" ? "bg-[hsl(230,80%,97%)] text-[hsl(230,80%,55%)] border-b-2 border-[hsl(230,80%,55%)]" : "text-[hsl(230,15%,50%)]"}`}
                >
                  <span className="w-5 h-5 rounded-full bg-[hsl(120,50%,45%)] text-white text-[10px] font-bold flex items-center justify-center">2</span>
                  Design
                </button>
              </div>
              ...
              <div className={`p-5 ${mobileStep !== "content" ? "hidden lg:block" : ""}`}>
                <div className="flex items-center gap-2 mb-4">
                  <span className="w-6 h-6 rounded-full bg-[hsl(230,80%,55%)] text-white text-[10px] font-bold flex items-center justify-center">1</span>
                  <h2 className="font-semibold text-sm text-[hsl(230,30%,15%)]">Complete the content</h2>
                </div>
                {renderInputFields()}
              </div>

              <hr className={`border-[hsl(230,20%,92%)] ${mobileStep !== "content" ? "hidden lg:block" : "hidden lg:block"}`} />

              {/* Step 2: Design (always visible on desktop, toggled on mobile) */}
              <div className={`p-5 ${mobileStep !== "design" ? "hidden lg:block" : ""}`}>
                <div className="flex items-center gap-2 mb-4">
                  <span className="w-6 h-6 rounded-full bg-[hsl(120,50%,45%)] text-white text-[10px] font-bold flex items-center justify-center">2</span>
                  <h2 className="font-semibold text-sm text-[hsl(230,30%,15%)]">Design your QR</h2>
                </div>

                {/* Design Sub-tabs */}
                <div className="flex gap-0.5 mb-4 border-b border-[hsl(230,20%,92%)]">
                  {[
                    ...
                    return (
                      <button key={tab.id} onClick={() => setDesignTab(tab.id)}
                        className={`flex items-center gap-1 px-3 py-2.5 text-sm font-medium transition-all border-b-2 -mb-[1px] ${designTab === tab.id ? "border-[hsl(230,80%,55%)] text-[hsl(230,80%,55%)]" : "border-transparent text-[hsl(230,15%,50%)] hover:text-[hsl(230,30%,25%)]"}`}
                      >
                    ...
                    <div className="grid grid-cols-3 sm:grid-cols-6 gap-2">
                      {FRAME_STYLES.map((f) => (
                        <button key={f.id} onClick={() => setFrameStyle(f.id)}
                          className={`p-3 rounded-xl border-2 text-center transition-all ${frameStyle === f.id ? "border-[hsl(230,80%,55%)] bg-[hsl(230,80%,97%)]" : "border-[hsl(230,20%,92%)] hover:border-[hsl(230,20%,80%)]"}`}
                        >
                          <div className="w-10 h-12 mx-auto mb-1 flex items-center justify-center">
                            {f.id === "none" ? (
                              <X className="w-5 h-5 text-[hsl(230,15%,60%)]" />
                            ) : (
                              <div className={`w-8 h-10 border-2 ${frameStyle === f.id ? "border-[hsl(230,80%,55%)]" : "border-[hsl(230,15%,70%)]"} ${f.id === "rounded" ? "rounded-lg" : f.id === "ticket" ? "rounded-sm" : ""} flex flex-col`}>
                                <div className="flex-1 flex items-center justify-center">
                                  <QrCode className="w-4 h-4 text-[hsl(230,15%,70%)]" />
                                </div>
                                <div className={`h-2.5 ${frameStyle === f.id ? "bg-[hsl(230,80%,55%)]" : "bg-[hsl(230,15%,70%)]"} ${f.id === "rounded" ? "rounded-b-md" : ""}`} />
                              </div>
                            )}
                          </div>
                          <span className="text-[10px] font-medium text-[hsl(230,15%,40%)]">{f.label}</span>
                        </button>
                      ))}
                    </div>
                    {frameStyle !== "none" && (
                      <div className="space-y-3 pt-2">
                        <div>
                          <Label className="text-sm text-[hsl(230,15%,40%)] mb-1.5 block">Frame Text</Label>
                          <Input value={frameText} onChange={(e) => setFrameText(e.target.value)} placeholder="SCAN ME" className="h-10 text-sm border-[hsl(230,20%,88%)]" maxLength={20} />
                        </div>
                        <div>
                          <Label className="text-sm text-[hsl(230,15%,40%)] mb-1.5 block">Frame Color</Label>
                          <div className="flex items-center gap-2">
                            <input type="color" value={frameColor} onChange={(e) => setFrameColor(e.target.value)} className="w-9 h-9 rounded-lg border border-[hsl(230,20%,88%)] cursor-pointer" />
                            <Input value={frameColor} onChange={(e) => setFrameColor(e.target.value)} className="flex-1 font-mono text-sm h-9 border-[hsl(230,20%,88%)]" maxLength={7} />
                          </div>
                        </div>
                      </div>
                    )}
                ...
                {designTab === "shape" && (
                  <div className="space-y-4">
                    {/* Shape sub-tabs */}
                    <div className="flex gap-1 bg-[hsl(230,20%,96%)] rounded-lg p-0.5">
                      <button onClick={() => setShapeSubTab("style")}
                        className={`flex-1 px-3 py-1.5 rounded-md text-xs font-medium transition-all ${shapeSubTab === "style" ? "bg-white text-[hsl(230,80%,55%)] shadow-sm" : "text-[hsl(230,15%,50%)]"}`}
                      >Shape style</button>
                      <button onClick={() => setShapeSubTab("edge")}
                        className={`flex-1 px-3 py-1.5 rounded-md text-xs font-medium transition-all ${shapeSubTab === "edge" ? "bg-white text-[hsl(230,80%,55%)] shadow-sm" : "text-[hsl(230,15%,50%)]"}`}
                    ...
                    {shapeSubTab === "style" ? (
                      <>
                        <div>
                          <Label className="text-xs text-[hsl(230,15%,40%)] mb-2 block font-semibold">Shape style</Label>
                          <div className="grid grid-cols-5 sm:grid-cols-9 gap-1.5">
                            {DOT_STYLES.map((s) => (
                              <button key={s.id} onClick={() => setDotStyle(s.id)}
                                className={`aspect-square rounded-lg border-2 flex items-center justify-center transition-all ${dotStyle === s.id ? "border-[hsl(230,80%,55%)] bg-[hsl(230,80%,97%)]" : "border-[hsl(230,20%,92%)] hover:border-[hsl(230,20%,80%)]"}`}
                              >
                        ...
                        <div className="bg-[hsl(230,20%,97%)] rounded-xl p-3 space-y-3">
                          <div className="flex gap-4">
                            <div className="flex-1">
                              <Label className="text-xs text-[hsl(230,15%,40%)] mb-1.5 block">Border colour</Label>
                              <div className="flex items-center gap-1.5">
                                <Input value={fgColor} onChange={(e) => setFgColor(e.target.value)} className="font-mono text-xs h-8 border-[hsl(230,20%,88%)]" maxLength={7} />
                                <input type="color" value={fgColor} onChange={(e) => setFgColor(e.target.value)} className="w-8 h-8 rounded border border-[hsl(230,20%,88%)] cursor-pointer" />
                              </div>
                            </div>
                            <div className="flex-1">
                              <Label className="text-xs text-[hsl(230,15%,40%)] mb-1.5 block">Background colour</Label>
                              <div className="flex items-center gap-1.5">
                                <Input value={bgColor} onChange={(e) => setBgColor(e.target.value)} className="font-mono text-xs h-8 border-[hsl(230,20%,88%)]" maxLength={7} />
                                <input type="color" value={bgColor} onChange={(e) => setBgColor(e.target.value)} className="w-8 h-8 rounded border border-[hsl(230,20%,88%)] cursor-pointer" />
                              </div>
                            </div>
                          </div>
                          <button onClick={invertColors} className="flex items-center gap-1.5 text-[hsl(230,80%,55%)] text-xs font-medium hover:underline">
                      ...
                      <>
                        <div>
                          <Label className="text-xs text-[hsl(230,15%,40%)] mb-2 block font-semibold">Corner style</Label>
                          <div className="grid grid-cols-4 gap-2">
                            {CORNER_STYLES.map((c) => (
                              <button key={c.id} onClick={() => setCornerStyle(c.id)}
                                className={`p-3 rounded-xl border-2 text-center transition-all ${cornerStyle === c.id ? "border-[hsl(230,80%,55%)] bg-[hsl(230,80%,97%)]" : "border-[hsl(230,20%,92%)] hover:border-[hsl(230,20%,80%)]"}`}
                              >
                                <CornerPreview style={c.id} active={cornerStyle === c.id} />
                                <span className="text-[10px] font-medium text-[hsl(230,15%,40%)] mt-1 block">{c.label}</span>
                              </button>
                            ))}
                          </div>
                        </div>
                        <div>
                          <Label className="text-xs text-[hsl(230,15%,40%)] mb-2 block">Size: {qrSize}px</Label>
                          <Slider value={[qrSize]} onValueChange={(v) => setQrSize(v[0])} min={150} max={500} step={10} />
                        </div>
                        <div>
                          <Label className="text-xs text-[hsl(230,15%,40%)] mb-2 block">Margin: {margin}px</Label>
                ...
                {designTab === "logo" && (
                  <div className="space-y-4">
                    <Label className="text-xs text-[hsl(230,15%,40%)] font-semibold block">Select a logo</Label>
                      ...
                      {PRESET_LOGOS.map((logo) => {
                        ...
                        return (
                          <button key={logo.id}
                            ...
                            className={`w-12 h-12 rounded-full flex items-center justify-center transition-all border-2 ${isActive ? "border-[hsl(230,80%,55%)] scale-110" : "border-[hsl(230,20%,90%)] hover:border-[hsl(230,20%,75%)]"}`}
                    ...
                    <button onClick={() => logoInputRef.current?.click()}
                      className="w-full p-5 border-2 border-dashed border-[hsl(230,20%,82%)] rounded-xl hover:border-[hsl(230,80%,55%)] hover:bg-[hsl(230,80%,98%)] transition-all flex flex-col items-center gap-1.5 text-[hsl(230,15%,50%)]"
                    >
                      <Upload className="w-6 h-6" />
                      <span className="text-sm font-medium">Upload a logo</span>
                      <span className="text-[10px] text-[hsl(230,15%,60%)]">(JPG, JPEG, or PNG / 2MB max)</span>
                    </button>
                    ...
                    {logoFile && (
                      <div className="flex items-center gap-3 p-3 bg-[hsl(230,20%,96%)] rounded-xl">
                        ...
                        <div className="flex-1">
                          <p className="text-sm font-medium text-[hsl(230,30%,20%)]">Custom logo uploaded</p>
                          <p className="text-[10px] text-[hsl(230,15%,55%)]">Will appear in QR center</p>
                        </div>
                        <Button variant="ghost" size="icon" onClick={() => setLogoFile(null)} className="text-[hsl(0,60%,50%)] h-8 w-8">
                          <X className="w-4 h-4" />
                        </Button>
                      </div>
                    )}

                    <p className="text-[10px] text-[hsl(230,15%,60%)] bg-[hsl(45,80%,95%)] p-2 rounded-lg">
                ...
                {designTab === "level" && (
                  <div className="space-y-2">
                    <Label className="text-xs text-[hsl(230,15%,40%)] font-semibold block">Error Correction Level</Label>
                    {[
                      ...
                      <button key={lvl.id} onClick={() => setErrorLevel(lvl.id)}
                        className={`w-full flex items-center gap-3 p-3 rounded-xl border-2 transition-all text-left ${errorLevel === lvl.id ? "border-[hsl(230,80%,55%)] bg-[hsl(230,80%,97%)]" : "border-[hsl(230,20%,92%)] hover:border-[hsl(230,20%,80%)]"}`}
                      >
                        <div className={`w-4.5 h-4.5 rounded-full border-2 flex items-center justify-center ${errorLevel === lvl.id ? "border-[hsl(230,80%,55%)]" : "border-[hsl(230,20%,80%)]"}`}>
                          {errorLevel === lvl.id && <div className="w-2 h-2 rounded-full bg-[hsl(230,80%,55%)]" />}
                        </div>
                        <div>
                          <p className={`text-sm font-medium ${errorLevel === lvl.id ? "text-[hsl(230,80%,55%)]" : "text-[hsl(230,30%,20%)]"}`}>{lvl.label}</p>
                          <p className="text-[10px] text-[hsl(230,15%,55%)]">{lvl.desc}</p>
                        </div>
                      </button>
                    ))}
                  </div>
            ...
            {/* Right Panel: Preview */}
            <div className="w-full lg:w-[300px] p-5 bg-[hsl(230,20%,98%)]">
              <div className="flex items-center gap-2 mb-3">
                <span className="w-6 h-6 rounded-full bg-[hsl(230,80%,55%)] text-white text-[10px] font-bold flex items-center justify-center">3</span>
                <h2 className="font-semibold text-sm text-[hsl(230,30%,15%)]">Download your QR</h2>
              </div>

              {/* Preview Card */}
              <div className="bg-white rounded-2xl border border-[hsl(230,20%,92%)] p-4 flex items-center justify-center min-h-[200px] mb-4 shadow-sm">
                {qrDataUrl ? (
                  ...
                  <div className="text-center py-4">
                    <QrCode className="w-16 h-16 mx-auto text-[hsl(230,20%,85%)] mb-2" />
                    <p className="text-xs text-[hsl(230,15%,55%)]">Your QR will appear here</p>
                  </div>
                )}
              </div>
              ...
              <div className="relative mb-3">
                <Button onClick={() => qrDataUrl ? setShowDownloadMenu(!showDownloadMenu) : null}
                  disabled={!qrDataUrl}
                  className="w-full h-10 bg-[hsl(230,80%,55%)] hover:bg-[hsl(230,80%,48%)] text-white font-medium rounded-xl text-sm"
                >
                ...
                {showDownloadMenu && (
                  <div className="absolute top-full left-0 right-0 mt-1 bg-white rounded-xl shadow-lg border border-[hsl(230,20%,90%)] overflow-hidden z-10">
                    <button onClick={() => downloadFile("png")} className="w-full px-4 py-2.5 text-sm text-left hover:bg-[hsl(230,20%,97%)] flex items-center gap-2">
                      <Download className="w-3.5 h-3.5" /> Download PNG
                    </button>
                    <button onClick={() => downloadFile("svg")} className="w-full px-4 py-2.5 text-sm text-left hover:bg-[hsl(230,20%,97%)] flex items-center gap-2 border-t border-[hsl(230,20%,92%)]">
              ...
              <div className="flex gap-2">
                <Button onClick={copyQrImage} disabled={!qrDataUrl} variant="outline" className="flex-1 h-9 rounded-xl border-[hsl(230,20%,88%)] text-[hsl(230,15%,40%)] text-xs">
                  <Copy className="w-3.5 h-3.5 mr-1" /> Copy
                </Button>
                <Button onClick={shareQr} disabled={!qrDataUrl} variant="outline" className="flex-1 h-9 rounded-xl border-[hsl(230,20%,88%)] text-[hsl(230,15%,40%)] text-xs">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
          {[
            ...
            <div key={i} className="bg-white rounded-xl border border-[hsl(230,20%,92%)] p-4 text-center">
              <t.icon className="w-8 h-8 text-[hsl(230,80%,55%)] mx-auto mb-1.5" />
              <p className="font-semibold text-sm text-[hsl(230,30%,15%)]">{t.title}</p>
              <p className="text-[10px] text-[hsl(230,15%,55%)] mt-0.5">{t.desc}</p>
            </div>
          ))}
        </div>
      </div>
...
const DotPreview = ({ style, active }: { style: DotStyle; active: boolean }) => {
  const color = active ? "hsl(230,80%,55%)" : "hsl(230,15%,30%)";
  const size = 6;
  const gap = 2;
...
const CornerPreview = ({ style, active }: { style: CornerStyle; active: boolean }) => {
  const color = active ? "hsl(230,80%,55%)" : "hsl(230,15%,40%)";
  return (
